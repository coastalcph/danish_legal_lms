#!/usr/bin/env python
# coding=utf-8
""" Finetuning models on Danish EUR-LEX (e.g. Danish-RoBERTa, Danish-LegalBERT)."""
import json
import logging
import os
import random
...
class DataTrainingArguments:
    ...
    the command line.
    """
    concept_level: Optional[str] = field(
        default='level_2',
        metadata={
...
class ModelArguments:
    ...
    use_spectral_decoupling: bool = field(
        default=False, metadata={"help": "Whether to Spectral Decoupling penalty or not."}
    )
...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("multi_eurlex", "da", split="train", label_level=data_args.concept_level,
                                     cache_dir=model_args.cache_dir, use_auth_token=AUTH_KEY)

    if training_args.do_eval:
        eval_dataset = load_dataset("multi_eurlex", "da", split="validation", label_level=data_args.concept_level,
                                    cache_dir=model_args.cache_dir, use_auth_token=AUTH_KEY)
    ...
    with open(os.path.join(DATA_DIR, 'eurovoc_descriptors.json')) as file:
        eurovoc_descriptors = json.load(file)
    ...
    config.use_spectral_decoupling = model_args.use_spectral_decoupling
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            with open(report_predict_file, "w") as writer:
                writer.write(classification_report(y_true=labels, y_pred=hard_predictions,
