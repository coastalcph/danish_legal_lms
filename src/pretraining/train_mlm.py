...
def main():
    ...
    if os.path.exists(os.path.join(model_args.model_name_or_path, "pytorch_model.bin")) or url_code == 200:
        logger.info("Continued training from LM checkpoint!")
        ...
        if last_checkpoint is not None:
            logger.info(f"Continue training LM from checkpoint {last_checkpoint}!")
        else:
            logger.info("Training new LM from scratch!")
    ...
    if data_args.line_by_line:
        ...
        logger.info(f"Reading documents one-by-one up to {max_seq_length}!!!")
